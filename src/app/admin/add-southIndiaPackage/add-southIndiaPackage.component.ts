...
@Component({
  selector: 'app-add-southIndiaPackage',
  templateUrl: './add-southIndiaPackage.component.html',
  styleUrls: ['./add-southIndiaPackage.component.css']
})
export class AddSouthIndiaPackageComponent implements OnInit {
  ...
  submit(data: IPackage){
    this.packageService.addSouthPackage(data).subscribe((result:any)=>{
      console.log(result)
      if(result){
        ...
        setTimeout(()=>{
          ...
          this.router.navigate([`admin/addSouthItinerary/${nextId}/`]);
        },1000)
      }
    })
