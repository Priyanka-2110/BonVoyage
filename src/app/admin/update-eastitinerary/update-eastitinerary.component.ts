...
@Component({
  selector: 'app-update-eastitinerary',
  templateUrl: './update-eastitinerary.component.html',
  styleUrls: ['./update-eastitinerary.component.css']
})
export class UpdateEastitineraryComponent implements OnInit {
  ...
  updatePackageData() {
    if (this.packageData) {
      ...
      this.packageService.updateEastItinerary(this.packageData).subscribe((result) => {
        if (result) {
          this.packageMessage = "Package has been updated successfully";
          setTimeout(() => {
            const nextId = result.id;
            this.router.navigate([`admin/updateEastGallery/${nextId}/`]);
          }, 1000);
        }
      });
    }
