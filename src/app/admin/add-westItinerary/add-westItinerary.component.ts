...
@Component({
  selector: 'app-add-westItinerary',
  templateUrl: './add-westItinerary.component.html',
  styleUrls: ['./add-westItinerary.component.css']
})
export class AddWestItineraryComponent implements OnInit {
  ...
  submit(data: IPackage){
    ...
    this.packageService.addWestItinerary(data).subscribe((result)=>{
      console.log(result)
      if(result){
        ...
        setTimeout(()=>{
          ...
          this.router.navigate([`admin/addWestGallery/${nextId}/`])
        },1000)
      }
    })
