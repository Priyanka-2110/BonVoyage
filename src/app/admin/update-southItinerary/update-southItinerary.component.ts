...
@Component({
  selector: 'app-update-southItinerary',
  templateUrl: './update-southItinerary.component.html',
  styleUrls: ['./update-southItinerary.component.css']
})
export class UpdateSouthItineraryComponent implements OnInit {
      ...
      this.packageService.updateSouthItinerary(this.packageData).subscribe((result) => {
        if (result) {
          ...
          setTimeout(() => {
            ...
            this.router.navigate([`admin/updateSouthGallery/${nextId}/`]);
          }, 1000);
        }
      });
    }
